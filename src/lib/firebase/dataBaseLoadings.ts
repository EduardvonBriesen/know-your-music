...
import {MAX_HISTORY_LENGTH} from "./dataBaseTypeDefinitions";
import type { UserData, Genre, Levels, LevelData } from "./dataBaseTypeDefinitions";
import {    newHistoryArrayElement,newSessionsArrayElement, 
            getGenreForItemtype, getLevelForGenre, 
            getUpdatedHistoryListOfGenres, getUpdatedScores} from "./dataBaseHelpers";   
...
export const saveHistory = async (docName: string, db: Firestore)=>{
    ...
    if (docSnap.exists()) {
        try{
            ...
            const date = new Date();
            //console.log(Math.round(begin.valueOf()/1000));
...
export async function updateUserProgressData(db: Firestore, docName: string, relativePoints: number, genre: Genre, level: Levels){
    const collectionsName = "users";
    const docRef = doc(db, collectionsName, docName);
    const docSnap = await getDoc(docRef);
    if (!docSnap.exists()) {
